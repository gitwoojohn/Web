' Name: Robust Office Inventory Scan - Version 1.9.0 
...
Dim dicProductCodeC2R, dicActiveC2RVersions 
...
Function GetLicenseData(iPosMaster, iVersionMajor, ByVal sConfigName, iLicPos, sPossibleSkusFull) 
    On Error Resume Next 
    Dim arrLicData (22) 
    Dim iPropCnt, iLicCnt, iLeft, iIndex 
    Dim ProdLic, prop, ConfigProd, ProtectionClass, PLProp 
            ...
            For Each ConfigProd in arrLic 
                ...
                If Len(ProdLic.Name) > iLeft -1 Then 
                    If (LCase(Left(ProdLic.Name, iLeft)) = LCase(sPrefix & ConfigProd) OR fMsiMatchesOsppID) AND (iLicCnt > iLicPos) AND InStr(sPossibleSkusFull, Prodlic.Name) > 0 Then  
                        For Each PLProp in ProdLic.Properties_ 
                            Select Case LCase(PLProp.Name) 
                            Case LCase("VLActivationTypeEnabled") 
                                arrLicData(OSPP_ACTIVATIONTYPE) = PLProp.Value 
                            Case LCase("ID") 
                                arrLicData(OSPP_ID) = PLProp.Value 
                            Case LCase("ApplicationId") 
                                arrLicData(OSPP_APPLICATIONID) = PLProp.Value 
                            Case LCase("PartialProductKey") 
                                arrLicData(OSPP_PARTIALPRODUCTKEY) = PLProp.Value 
                            Case LCase("Description") 
                                arrLicData(OSPP_DESCRIPTION) = PLProp.Value 
                            Case LCase("Name") 
                                arrLicData(OSPP_NAME) = PLProp.Value 
                            Case LCase("LicenseStatus") 
                                arrLicData(OSPP_LICENSESTATUS) = PLProp.Value 
                            Case LCase("LicenseStatusReason") 
                                arrLicData(OSPP_LICENSESTATUSREASON) = PLProp.Value 
                            Case LCase("GracePeriodRemaining") 
                                arrLicData(OSPP_GRACEPERIODREMAINING) = PLProp.Value 
                            Case LCase("LicenseFamily") 
                                arrLicData(OSPP_LICENSEFAMILY) = PLProp.Value 
                            Case LCase("DiscoveredKeyManagementServiceMachineName") 
                                arrLicData(OSPP_DISCOVEREDKEYMANAGEMENTSERVICEMACHINENAME) = PLProp.Value 
                            Case LCase("KeyManagementServicePort") 
                                arrLicData(OSPP_KEYMANAGEMENTSERVICEPORT) = PLProp.Value 
                            Case LCase("VLActivationInterval") 
                                arrLicData(OSPP_VLACTIVATIONINTERVAL) = PLProp.Value 
                            Case LCase("ProductKeyID") 
                                arrLicData(OSPP_PRODUCTKEYID) = PLProp.Value 
                            Case LCase("ProductKeyID2") 
                                arrLicData(OSPP_PRODUCTKEYID2) = PLProp.Value 
                                If Len(PLProp.Value) > 18 Then 
                                    sTmp = Right (Replace (arrLicData(OSPP_PRODUCTKEYID2), "-", ""), 19) 
                                    arrLicData(OSPP_MACHINEKEY) = Mid (sTmp, 1, 5) & "-" & Mid (sTmp, 6, 3) & "-" & Mid (sTmp, 9, 6) 
                                End If 
                            Case LCase("DiscoveredKeyManagementServiceMachinePort") 
                                arrLicData(OSPP_DISCOVEREDKEYMANAGEMENTSERVICEMACHINEPORT) = PLProp.Value 
                            Case LCase("KeyManagementServiceLookupDomain") 
                                arrLicData(OSPP_KEYMANGEMENTSERVICELOOKUPDOMAIN) = PLProp.Value 
                            Case LCase("ADActivationObjectName") 
                                arrLicData(OSPP_ADACTIVATIONOBJECTNAME) = PLProp.Value 
                            Case LCase("ADActivationObjectDN") 
                                arrLicData(OSPP_ADACTIVATIONOBJECTDN) = PLProp.Value 
                            Case LCase("ADActivationCsvlkPid") 
                                arrLicData(OSPP_ADACTIVATIONCSVLPID) = PLProp.Value 
                            Case LCase("ADActivationCsvlkSkuId") 
                                arrLicData(OSPP_ADACTIVATIONCSVLSKUID) = PLProp.Value 
                            Case Else 
                            End Select 
                        Next 'PLProp 
                        iLicPos = iLicCnt 
...
Sub FindV2VirtualizedProducts 
    ...
    If RegEnumKey(HKLM, REG_C2RPRODUCTIDS & sActive, arrConfigProducts) Then 
        For Each prod In arrConfigProducts 
            ...
            Case Else 
                ...
                If NOT dicVirt2ConfigID.Exists(sProd) Then 
                    If fv2SxS Then 
                        If InStr(sProd, ".15") > 0 Then 
                            sProd = Left(sProd, InStr(sProd, ".15") - 1) 
                            If NOT dicVirt2ConfigID.Exists(sProd) Then 
    ...
    If fv2SxS Then 
        ...
        If RegReadStringValue(HKLM, REG_C2RPRODUCTIDS & sActive & "\culture\x-none.15\", "Version", sVersionFallback) Then 
            dicC2RPropV2.Add STR_VERSION, sVersionFallback 
        End If 
         
        'Cultures 
        If RegEnumKey(HKLM, REG_C2RPRODUCTIDS & sActive & "\culture", arrCultures) Then 
            For Each culture in arrCultures 
                sCult = culture 
                If InStr(sCult, sSxsExt) > 0 Then sCult = Left(sCult, InStr(sCult, sSxsExt) - 1) 
    ...
    Else 
        'Shared ProductVersion 
        If RegReadStringValue(HKLM, REG_C2RPRODUCTIDS & sActive & "\culture\", sXNone, sVersionFallback) Then 
            dicC2RPropV2.Add STR_VERSION, sVersionFallback 
        End If 
         
        'Cultures 
        If RegEnumValues (HKLM, REG_C2RPRODUCTIDS & sActive & "\culture", arrCultures, arrTypes) Then 
            For Each culture in arrCultures 
                sCult = culture 
                Select Case LCase(sCult) 
                Case sXNone 
    ...
    For Each ArpItem in dicArp.Keys 
        ...
        If InStr(LCase(sValue), "microsoft office 15") > 0 OR _ 
           (InStr(LCase(sValue), "productstoremove=") > 0 AND InStr(sValue, ".15_") > 0 ) Then 
    ...
    If dicVirt2Prod.Count > 0 Then 
        ...
        'SCA 
        If RegReadValue (HKLM, REG_C2RCONFIGURATION, "SharedComputerLicensing", sValue, "REG_DWORD") Then 
        ...
        'CDNBaseUrl 
        If RegReadValue(HKLM, REG_C2RCONFIGURATION & sSxsPath, "CDNBaseUrl", sValue, "REG_SZ") Then 
            ...
            End If  
        ElseIf RegReadValue(HKLM, REG_C2RSCENARIO & "INSTALL\15.0", "BaseUrl", sValue, "REG_SZ") Then 
            dicC2RPropV2.Add STR_CDNBASEURL, sValue 
            dicC2RPropV2.Add hAtS(Array("49","6E","73","74","61","6C","6C","53","6F","75","72","63","65","20","43","68","61","6E","6E","65","6C")), GetBr(sValue) 
            'Last known InstallSource used 
            dicC2RPropV2.Add STR_LASTUSEDBASEURL, sValue 
        ...
        Else 
            If fv2SxS Then 
                dicC2RPropV2.Add STR_CDNBASEURL, "Default" 
            Else 
        ...
        'UpdatesEnabled 
        If RegReadValue (HKLM, REG_C2RCONFIGURATION & sSxsPath, "UpdatesEnabled", sValue, "REG_SZ") Then 
        ...
        If RegReadValue (HKLM, REG_C2RUPDATEPOL, "enableautomaticupgrade", sValue, "REG_DWORD") Then 
        ...
        'UpdateUrl / Path 
        If RegReadValue (HKLM, REG_C2RCONFIGURATION & sSxsPath, "UpdateUrl", sValue, "REG_SZ") Then 
            dicC2RPropV2.Add STR_UPDATELOCATION, sValue 
        ElseIf RegReadValue (HKLM, REG_C2RCONFIGURATION & sSxsPath, "UpdateChannel", sValue, "REG_SZ") Then 
            dicC2RPropV2.Add STR_UPDATELOCATION, sValue 
        ...
        dicC2RPropV2.Add STR_USEDUPDATELOCATION, sValue 
         ...
         'UpdateVersion 
        If RegReadValue (HKLM, REG_C2RCONFIGURATION & sSxsPath, "UpdateToVersion", sValue, "REG_SZ") Then 
...
'                dicC2RPropV2.Add STR_UPDATETOVERSION, sValue 
        ...
        For Each VProd in dicVirt2Prod.Keys 
            ...
            If NOT fv2SxS Then 
                'Child Packages 
                sCurKeyL0 = REG_C2RPRODUCTIDS & sActive & "\" 
...
Sub FindV3VirtualizedProducts 
    ...
    For Each ArpItem in dicArp.Keys 
        ...
        sCurKey = REG_ARP & ArpItem & "\" 
        fUninstallString = RegReadValue(HKLM, sCurKey, "UninstallString", sValue, "REG_SZ") 
        If (InStr(LCase(sValue), "productstoremove=") > 0 AND InStr(sValue, ".16_") > 0  ) Then 
    ...
    If dicVirt3Prod.Count > 0 Then 
        ...
        If RegReadValue(HKLM, REG_C2RCONFIGURATION, "CDNBaseUrl", sValue, "REG_SZ") Then 
            ...
            End If  
        ElseIf RegReadValue(HKLM, REG_C2RSCENARIO & "INSTALL", "BaseUrl", sValue, "REG_SZ") Then 
            dicC2RPropV3.Add STR_CDNBASEURL, sValue 
            dicC2RPropV3.Add hAtS(Array("49","6E","73","74","61","6C","6C","53","6F","75","72","63","65","20","43","68","61","6E","6E","65","6C")), GetBr(sValue) 
            'Last known InstallSource used 
            dicC2RPropV3.Add STR_LASTUSEDBASEURL, sValue 
        Else 
            If fv2SxS Then 
                dicC2RPropV3.Add STR_CDNBASEURL, "Default" 
            Else 
        ...
        'UpdatesEnabled Policy 
        If RegReadValue (HKLM, REG_C2RUPDATEPOL, "enableautomaticupdates", sValue, "REG_DWORD") Then 
        ...
        'UpdateChannel  
        If RegReadValue (HKLM, REG_C2RCONFIGURATION, "UpdateChannel", sValue, "REG_SZ") Then 
            dicC2RPropV3.Add STR_UPDATECHANNEL, sValue 
        ElseIf RegReadValue (HKLM, REG_C2RCONFIGURATION, "UpdateBranch", sValue, "REG_SZ") Then 
            dicC2RPropV3.Add STR_UPDATECHANNEL, sValue 
        Else 
        ...
        'UpdateHideEnableDisableUpdates Policy 
        If RegReadValue (HKLM, REG_C2RUPDATEPOL, "hideenabledisableupdates ", sValue, "REG_DWORD") Then 
        ...
        'OfficeMgmtCOM 
        'GetGroupPolicyBooleanValue(C2R_MGMTCOM_POLICY 
        ...
        For Each key in arrKeys 
            ...
            Case "15.0" 
                If RegReadValue (HKLM, "Software\Microsoft\Office\" & key & "\ClickToRun\ProductReleaseIDs\Active\culture", "x-none", sValue, "REG_SZ") Then 
                    If Not dicActiveC2RVersions.Exists(sValue) Then dicActiveC2RVersions.Add sValue, Left(sValue, 2) 
                End If 
            Case "clicktorun" 
